...
# Import CEO learning system
from ..agents.ceo.ceo_agent import ceo_agent
from ..agents.ceo.ceo_learning_system import ceo_learning_system, initialize_first_learning
...
# Initialize CEO learning system
initialize_first_learning()
logger.info("CEO Learning System initialized")
...
async def ceo_analyze_and_prepare_context(agent_result: AgentExecutionResponse, next_agent_id: str, original_task: str, request_id: str) -> str:
    """CEO analyzes agent output and prepares context for next agent using learning system."""
    ...
    # Use CEO learning system to prepare delegation
    try:
        # Convert agent result to dict format for learning system
        agent_response_dict = {
        ...
        # Use CEO agent to prepare delegation with learning
        delegation_info = ceo_agent.prepare_delegation_task(
            source_agent=agent_result.agent_id,
            target_agent=next_agent_id,
            original_task=original_task,
            source_agent_response=agent_response_dict
        )
        
        log_orchestration_event(
            request_id=request_id,
            event_type="ceo_learning_applied",
            agent_id=next_agent_id,
            message=f"CEO applied learning rule for {agent_result.agent_name} -> {next_agent['name']}",
            additional_data={
                "learning_applied": delegation_info["learning_applied"],
                "used_learning_rule": delegation_info["used_learning_rule"],
                "source_output_included": delegation_info["source_output_included"]
            }
        )
        
        return delegation_info["task"]
        
    except Exception as e:
        logger.error(f"CEO learning system failed, using fallback: {e}")
        
        # Fallback to original logic if learning system fails
        analysis_prompt = f"""
...
"""
        ...
        try:
            ...
            log_orchestration_event(
                ...
                agent_id=next_agent_id,
                message=f"CEO prepared context for {next_agent['name']} (fallback)",
                additional_data={
            ...
            logger.error(f"CEO context analysis fallback failed: {fallback_error}")
            # Final fallback: use original output with basic formatting
...
# Export router and learning components
__all__ = ["ceo_router", "ceo_agent", "ceo_learning_system"]


@ceo_router.get("/learning-insights")
async def get_ceo_learning_insights():
    """Get insights about CEO learning patterns and delegation rules."""
    try:
...
@ceo_router.post("/add-learning-rule")
async def add_ceo_learning_rule(
    source_agent: str,
    target_agent: str,
    learning_rule: str,
    context: Optional[Dict[str, Any]] = None
):
    """Add a new learning rule to the CEO learning system."""
    try:
        ...
        return {
            "status": "success",
            "message": f"Learning rule added: {source_agent} -> {target_agent}",
            "rule": learning_rule,
            "timestamp": datetime.now(timezone.utc).isoformat()
        }
    except Exception as e:
...
@ceo_router.get("/learning-status")
async def get_ceo_learning_status():
    """Get current status of CEO learning system."""
    try:
        stats = ceo_learning_system.get_learning_stats()
        return {
            "status": "active",
            "learning_stats": stats,
            "first_rule_initialized": True,
            "content_writer_to_social_media_rule": ceo_learning_system.should_pass_output("content_writer", "social_media_publisher"),
            "timestamp": datetime.now(timezone.utc).isoformat()
        }
    except Exception as e:
